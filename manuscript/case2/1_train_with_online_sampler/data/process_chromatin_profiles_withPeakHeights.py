...
def _format_feature_dataset(filepath, feature):
    file_df = pd.read_table(filepath, header=None, usecols=[0, 1, 2, 3, 6])
...
if __name__ == "__main__":
    ...
    full_aggregate_file.columns = ["chrom", "start", "end", "strand", "peak_height" ,"feature", "metadata_index"]
    output_file = os.path.join(output_dir, "deepsea_full_unsorted.bed")
    with open(output_file, 'w+') as file_handle:
        for row in full_aggregate_file.itertuples():
            file_handle.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format(row.chrom, row.start, row.end, row.peak_height, row.feature))
